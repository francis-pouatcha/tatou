...
public class PurchaseOrderFinder implements GpaoEntityFinder<PurchaseOrder> {
	...
	public List<PurchaseOrder> find(int page, int size) {
		return PurchaseOrder.findPurchaseOrderByStatusAndTenderRefAndDateBetween(supplier, orderState,tenderRef, beginDate, endDate, reference).
				setFirstResult(page).setMaxResults(page).getResultList();
	}
